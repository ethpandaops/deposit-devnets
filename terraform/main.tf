/////////////////////////////////////////////////////////////////////////////////////////
//
// Access tokens for the Digitalocean must be defined like:
...
variable "digitalocean_vpcs" {
  ...
  default = {
    "ams3" = {
      ip_range = "10.73.1.0/24"
    },
    "nyc1" = {
      ip_range = "10.72.2.0/24"
    },
  }
}
...
resource "digitalocean_vpc" "main" {
  ...
  name     = try(each.value.name, "${local.default_resource_prefix}-${var.ethereum_network}-${each.key}")
  region   = each.key
